-- 0017 pinned these functions to `search_path = public`, but `citext` lives in
-- the `extensions` schema (moved there by 0011 to keep it out of `public`), so
-- the `::citext` cast on people.email failed at runtime with
-- `type "citext" does not exist`. Admission was broken for every student.
--
-- Add `extensions` to the pinned path rather than dropping the cast: the path
-- stays explicit (not inherited from the caller, which is the point of pinning
-- it), and any future use of pgcrypto/citext in these functions resolves too.
