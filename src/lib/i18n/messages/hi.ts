import type { Messages } from "./en";

/** हिन्दी. Typed as a Partial of the English catalogue, so a missing key is a
 *  reportable gap rather than a build error. See `coverage.ts`. */
export const hi: Messages = {
  "app.name": "SchoolOS",
  "app.skipToContent": "मुख्य सामग्री पर जाएँ",
  "app.search": "खोजें",
  "app.commandPalette": "कमांड पैलेट",
  "app.navigation": "नेविगेशन",
  "app.sidebar.expand": "साइडबार खोलें",
  "app.sidebar.collapse": "साइडबार समेटें",
  "app.openNav": "नेविगेशन मेन्यू खोलें",
  "app.theme.toggle": "थीम बदलें",
  "app.theme.light": "उजला",
  "app.theme.dark": "गहरा",
  "app.theme.system": "सिस्टम के अनुसार",
  "app.signOut": "साइन आउट",
  "app.signedInAs": "{name} के रूप में साइन इन",
  "app.language": "भाषा",
  "palette.title": "खोज",
  "palette.description": "किसी छात्र, सहकर्मी या पुस्तक को ढूँढ़ें, या किसी पृष्ठ पर जाएँ",
  "palette.placeholder": "छात्र, कर्मचारी, पुस्तकें खोजें…",
  "palette.results": "परिणाम",
  "palette.admission": "प्रवेश क्रमांक {number}",
  "app.language.choose": "भाषा चुनें",
  "app.language.followSchool": "विद्यालय के अनुसार ({name})",
  "app.language.saved": "भाषा बदलकर {name} कर दी गई।",

  "nav.dashboard": "डैशबोर्ड",
  "nav.setup": "सेटअप",
  "nav.setupHint": "एक बार या साल में एक बार",
  "nav.assistant": "SchoolOS से पूछें",
  "nav.electives": "वैकल्पिक विषय",
  "nav.mySubjects": "मेरे विषय",
  "nav.students": "विद्यार्थी",
  "nav.attendance": "उपस्थिति",
  "nav.academics": "शैक्षणिक",
  "nav.timetable": "समय-सारणी",
  "nav.exams": "परीक्षाएँ",
  "nav.homework": "गृहकार्य",
  "nav.liveClasses": "लाइव कक्षाएँ",
  "nav.scanCard": "कार्ड स्कैन करें",
  "nav.biometric": "उपस्थिति रीडर",
  "nav.onlineTests": "ऑनलाइन परीक्षा",
  "nav.fees": "शुल्क",
  "nav.familyFees": "शुल्क",
  "nav.accounts": "लेखा",
  "nav.hr": "कर्मचारी",
  "nav.payroll": "वेतन",
  "nav.library": "पुस्तकालय",
  "nav.transport": "परिवहन",
  "nav.hostel": "छात्रावास",
  "nav.inventory": "भंडार",
  "nav.frontOffice": "कार्यालय",
  "nav.notifications": "सूचनाएँ",
  "nav.reports": "रिपोर्ट",
  "nav.checks": "ध्यान देने योग्य",
  "nav.settings": "सेटिंग्स",
  "nav.overview": "अवलोकन",
  "nav.people": "लोग",
  "nav.importStudents": "विद्यार्थी आयात",
  "nav.certificates": "प्रमाणपत्र",
  "nav.schedules": "स्वचालित संदेश",
  "nav.notices": "सूचना पट्ट",
  "nav.studentLeave": "विद्यार्थी अवकाश",
  "nav.cover": "स्थानापन्न व्यवस्था",
  "nav.schoolSettings": "विद्यालय सेटिंग्स",
  "nav.concessions": "शुल्क छूट",
  "nav.classRoutine": "कक्षा समय-सारणी",
  "nav.myWeek": "मेरा सप्ताह",
  "nav.attendanceReport": "उपस्थिति रिपोर्ट",
  "nav.promotion": "प्रोन्नति",
  "nav.studyMaterial": "अध्ययन सामग्री",
  "nav.reportCards": "रिपोर्ट कार्ड",
  "nav.busAssignments": "बस आवंटन",
  "nav.staff": "कर्मचारी",
  "nav.academicYears": "शैक्षणिक वर्ष",
  "nav.staffList": "कर्मचारी सूची",
  "nav.staffAttendance": "कर्मचारी उपस्थिति",
  "nav.leave": "अवकाश",
  "nav.salaryStructures": "वेतन संरचनाएँ",
  "nav.finance": "वित्त",
  "nav.feeCounter": "शुल्क काउंटर",
  "nav.balances": "शेष",
  "nav.invoices": "बिल",
  "nav.dayBook": "दैनिक बही",
  "nav.billingPeriods": "बिलिंग अवधि",
  "nav.feeSetup": "शुल्क व्यवस्था",
  "nav.voucherBook": "वाउचर बही",
  "nav.insight": "विश्लेषण",
  "nav.communication": "संचार",
  "nav.compose": "लिखें",
  "nav.deliveryLog": "वितरण लॉग",
  "nav.channels": "चैनल",
  "nav.catalog": "सूची",
  "nav.members": "सदस्य",
  "nav.issuesReturns": "निर्गम एवं वापसी",

  "common.save": "सहेजें",
  "common.saving": "सहेजा जा रहा है…",
  "common.saved": "सहेजा गया",
  "common.cancel": "रद्द करें",
  "common.delete": "हटाएँ",
  "common.edit": "संपादित करें",
  "common.add": "जोड़ें",
  "common.close": "बंद करें",
  "common.back": "पीछे",
  "common.next": "आगे",
  "common.previous": "पिछला",
  "common.confirm": "पुष्टि करें",
  "common.retry": "फिर कोशिश करें",
  "common.loading": "लोड हो रहा है…",
  "common.none": "कोई नहीं",
  "common.all": "सभी",
  "common.yes": "हाँ",
  "common.no": "नहीं",
  "common.required": "आवश्यक",
  "common.optional": "वैकल्पिक",
  "common.actions": "कार्रवाई",
  "common.status": "स्थिति",
  "common.date": "दिनांक",
  "common.name": "नाम",
  "common.total": "कुल",
  "common.notSet": "निर्धारित नहीं",

  "state.empty.title": "अभी यहाँ कुछ नहीं है",
  "state.error.title": "यह लोड नहीं हो सका",
  "state.error.body": "हमारी ओर से कुछ गड़बड़ हुई। दोबारा कोशिश करने पर आमतौर पर काम कर जाता है।",
  "state.noResults": "इस खोज से कुछ नहीं मिला।",
  "state.showingOf.one": "{total} में से {shown} पंक्ति",
  "state.showingOf.other": "{total} में से {shown} पंक्तियाँ",
  "state.selected.one": "{count} चयनित",
  "state.selected.other": "{count} चयनित",

  "login.title": "साइन इन करें",
  "login.subtitle": "वही पता उपयोग करें जो विद्यालय ने दिया है।",
  "login.email": "ईमेल पता",
  "login.password": "पासवर्ड",
  "login.submit": "साइन इन",
  "login.submitting": "साइन इन हो रहा है…",
  "login.forgot": "पासवर्ड भूल गए?",
  "login.showPassword": "पासवर्ड दिखाएँ",
  "login.brandHeadline": "प्रवेश, उपस्थिति, शुल्क और वह सब कुछ जो विद्यालय को चलाता है — एक ही प्रणाली में।",
  "login.brandSub": "उन लोगों के लिए बनाया गया जो विद्यालय चलाते हैं, केवल उनके लिए नहीं जो सॉफ़्टवेयर खरीदते हैं।",
  "login.copyright": "© {year} SchoolOS",
  "login.hidePassword": "पासवर्ड छिपाएँ",
  "login.failed": "यह ईमेल पता और पासवर्ड मेल नहीं खाते।",
  "login.noTenant":
    "यह लॉगिन अभी किसी विद्यालय से जुड़ा नहीं है। अपने प्रशासक से निमंत्रण भेजने को कहें।",


  // The DataTable's own chrome. It is on roughly twenty list screens, and
  // every string in it was English until these keys existed.
  "table.loading": "लोड हो रहा है…",
  "table.noResults": "कोई परिणाम नहीं",
  "table.showing": "{total} में से {from}–{to} दिखाए जा रहे हैं",
  "table.selected": "{total} में से {count} चुने गए",
  "table.rowsPerPage": "प्रति पृष्ठ पंक्तियाँ",
  "table.pageOf": "पृष्ठ {page} / {pages}",
  "table.firstPage": "पहला पृष्ठ",
  "table.previousPage": "पिछला पृष्ठ",
  "table.nextPage": "अगला पृष्ठ",
  "table.lastPage": "अंतिम पृष्ठ",
  "table.search": "खोजें…",
  "table.clearSearch": "खोज साफ़ करें",
  "table.savedViews": "सहेजे गए दृश्य",
  "table.noSavedViews": "अभी कोई सहेजा गया दृश्य नहीं",
  "table.nameThisView": "इस दृश्य को नाम दें",
  "table.save": "सहेजें",
  "table.deleteView": "दृश्य {name} हटाएँ",
  "table.export": "निर्यात",
  "table.columns": "स्तंभ",
  "table.emptyTitle": "यहाँ अभी कुछ नहीं है",
  "table.emptyDescription": "रिकॉर्ड बनने पर वे यहाँ दिखेंगे।",
  "table.errorTitle": "इसे लोड नहीं किया जा सका",
  "table.errorDescription": "हमारी ओर से कुछ गड़बड़ हुई।",
  "table.retry": "फिर कोशिश करें",
  "table.selectAll": "इस पृष्ठ की सभी पंक्तियाँ चुनें",
  "table.selectRow": "यह पंक्ति चुनें",

  "channels.title": "सूचना चैनल",
  "channels.subtitle": "संदेश वास्तव में बाहर जाएगा या नहीं, यह तीन बातों पर निर्भर है: इस बिल्ड में उस चैनल का ड्राइवर हो, विद्यालय ने उसे चालू करके पता दिया हो, और डिस्पैचर को उसकी कुंजियाँ मिली हों।",
  "channels.sending": "भेजा जा रहा है",
  "channels.holding": "रोका गया",
  "channels.noDriver": "कोई ड्राइवर नहीं",
  "channels.waiting": "प्रतीक्षारत",
  "channels.failed": "विफल",
  "channels.sentThisWeek": "इस सप्ताह भेजे गए",
  "channels.sendNow": "कतार के संदेश अभी भेजें",
  "channels.nothingWaiting": "भेजने के लिए कुछ भी प्रतीक्षारत नहीं है।",
  "channels.waitingCount.one": "{count} संदेश भेजे जाने की प्रतीक्षा में है।",
  "channels.waitingCount.other": "{count} संदेश भेजे जाने की प्रतीक्षा में हैं।",
  "channels.retryFailed": "{count} विफल पुनः भेजें",
  "channels.fromAddress": "प्रेषक पता",
  "channels.senderNumber": "प्रेषक नंबर",
  "channels.senderName": "प्रेषक का नाम",
  "channels.saveSender": "प्रेषक सहेजें",
  "channels.onForSchool": "इस विद्यालय के लिए चालू",
  "channels.offForSchool": "इस विद्यालय के लिए बंद",
  "channels.devices": "पंजीकृत उपकरण",
  "channels.devices.body": "पुश सूचना कहाँ जाएगी। टोकन यहाँ या कहीं और कभी नहीं दिखाए जाते — पुश टोकन एक अधिकार है, पता नहीं, इसलिए केवल संख्याएँ पढ़ी जा सकती हैं।",
  "channels.devices.none": "अभी किसी ने उपकरण पंजीकृत नहीं किया है, इसलिए हर पुश संदेश भेजा नहीं, रखा जाता है।",

  "settings.language.title": "भाषा",
  "settings.language.body":
    "यह अनुप्रयोग आपसे किस भाषा में बात करेगा। इससे किसी और के लिए कुछ नहीं बदलता, और सूचना किस भाषा में लिखी जाएगी यह भी नहीं बदलता — वह भेजने वाला तय करता है।",
  "settings.language.rtlNote": "उर्दू दाएँ से बाएँ लिखी जाती है, और पूरा इंटरफ़ेस उसके साथ घूम जाता है।",

  // --- the family's own screens -------------------------------------------
  "family.fees.title": "शुल्क",
  "family.fees.body":
    "स्कूल ने आपके प्रत्येक बच्चे के लिए जो शुल्क बनाया है और जितना बाकी है। बिल, रसीदें और हर समायोजन देखने के लिए नाम पर खोलें।",
  "family.fees.owing": "बाकी",
  "family.fees.settled": "चुकता",
  "family.fees.inCredit": "जमा शेष",
  "family.fees.notBilled": "बिल नहीं",
  "family.fees.nothingBilled": "इस वर्ष अब तक कोई शुल्क नहीं बना",
  "family.fees.lastPayment": "अंतिम भुगतान {date}",
  "family.fees.noPaymentYet": "अभी तक कोई भुगतान नहीं",
  "family.fees.footnote":
    "आंकड़े चालू शैक्षणिक वर्ष के हैं। पिछले वर्ष का बकाया खाते पर ही दिखता है, इसलिए कुछ चुपचाप नहीं छूटता।",
  "family.fees.noChildren.title": "इस लॉगिन से कोई बच्चा जुड़ा नहीं है",
  "family.fees.noChildren.body":
    "स्कूल कार्यालय से अपना खाता अपने बच्चे के रिकॉर्ड से जोड़ने को कहें।",
  "family.fees.staff.title": "यह पृष्ठ परिवार के लिए है",
  "family.fees.staff.body":
    "यह अभिभावक या छात्र को उनके अपने खाते दिखाता है। भुगतान लेने या किसी भी बच्चे को खोजने के लिए शुल्क काउंटर का उपयोग करें।",

  // --- the notice board ----------------------------------------------------
  "notices.title": "सूचना पट",
  "notices.body":
    "परिपत्र और घोषणाएँ, जो बाद में फिर पढ़ी जा सकें। आपको केवल वही दिखती हैं जो आपके लिए हैं।",
  "notices.manage": "सूचनाएँ लिखें और प्रबंधित करें",
  "notices.empty.title": "पट पर कुछ नहीं है",
  "notices.empty.staff":
    "अभी तक कुछ प्रकाशित नहीं हुआ। घोषणा के बाद भी सूचना यहीं रहती है, ताकि फिर पढ़ी जा सके।",
  "notices.empty.reader": "इस समय आपके लिए कोई सूचना नहीं है।",
  "notices.writeFirst": "पहली सूचना लिखें",
  "notices.pinned": "पिन किया गया",
  "notices.read": "पढ़ी गई",
  "notices.new": "नई",
  "notices.attachments.one": "{count} संलग्नक",
  "notices.attachments.other": "{count} संलग्नक",
  "notices.category.general": "सामान्य",
  "notices.category.circular": "परिपत्र",
  "notices.category.event": "आयोजन",
  "notices.category.examination": "परीक्षा",
  "notices.category.holiday": "अवकाश",
  "notices.category.urgent": "अत्यावश्यक",
  "guardians.relationship.father": "पिता",
  "guardians.relationship.mother": "माता",
  "guardians.relationship.guardian": "अभिभावक",
  "guardians.relationship.other": "अन्य",

  // --- values a family reads on a badge ------------------------------------
  "attendance.status.present": "उपस्थित",
  "attendance.status.absent": "अनुपस्थित",
  "attendance.status.late": "विलंब से",
  "attendance.status.excused": "अवकाश स्वीकृत",

  "leave.kind.sick": "बीमारी",
  "leave.kind.family": "पारिवारिक कारण",
  "leave.kind.travel": "यात्रा",
  "leave.kind.other": "अन्य",
  "leave.status.pending": "प्रतीक्षारत",
  "leave.status.approved": "स्वीकृत",
  "leave.status.refused": "अस्वीकृत",
  "leave.status.cancelled": "रद्द",

  "exams.kind.unit": "इकाई परीक्षा",
  "exams.kind.term": "सत्र परीक्षा",
  "exams.kind.half_yearly": "अर्धवार्षिक",
  "exams.kind.annual": "वार्षिक",
  "exams.kind.practical": "प्रायोगिक",
  "exams.kind.other": "अन्य",
  "exams.result.pass": "उत्तीर्ण",
  "exams.result.fail": "अनुत्तीर्ण",
  "exams.result.incomplete": "अपूर्ण",

  "homework.submission.pending": "जमा नहीं किया",
  "homework.submission.submitted": "जमा कर दिया",
  "homework.submission.graded": "जाँचा गया",
  "homework.submission.returned": "लौटाया गया",
  "material.kind.document": "फ़ाइल",
  "material.kind.video": "वीडियो",
  "material.kind.link": "लिंक",

  "fees.entryType.payment": "भुगतान",
  "fees.entryType.discount": "छूट",
  "fees.entryType.write_off": "बट्टे खाते में",
  "fees.entryType.fine": "जुर्माना",
  "fees.entryType.refund": "वापसी",
  "fees.method.cash": "नकद",
  "fees.method.cheque": "चेक",
  "fees.method.card": "कार्ड",
  "fees.method.upi": "यूपीआई",
  "fees.method.netbanking": "नेट बैंकिंग",
  "fees.method.bank_transfer": "बैंक हस्तांतरण",
  "fees.method.online": "ऑनलाइन गेटवे",
  "attendance.marked": "{name} — {status}",
  "academics.weekday.open": "{day} अब कार्यदिवस है",
  "academics.weekday.closed": "{day} अब बंद है",
  "channel.in_app": "ऐप में",
  "channel.email": "ईमेल",
  "channel.sms": "एसएमएस",
  "channel.whatsapp": "व्हाट्सऐप",
  "channel.push": "पुश",
  "delivery.status.queued": "कतार में",
  "delivery.status.sending": "भेजा जा रहा है",
  "delivery.status.sent": "भेज दिया",
  "delivery.status.failed": "विफल",
  "delivery.status.skipped": "छोड़ा गया",
  "audience.kind.all": "लॉगिन वाले सभी",
  "audience.kind.role": "एक भूमिका",
  "audience.kind.section": "एक कक्षा",
  "audience.kind.users": "नामित व्यक्ति",
  "audience.role": "भूमिका: {role}",
  "audience.section": "एक कक्षा — {who}",
  "audience.who.students": "छात्र",
  "audience.who.parents": "अभिभावक",
  "audience.who.both": "छात्र और अभिभावक",
  "audience.namedPeople.one": "{count} नामित व्यक्ति",
  "audience.namedPeople.other": "{count} नामित व्यक्ति",
  "timetable.period": "कालांश {n}",
  "timetable.periodBare": "कालांश",
  "cover.reason.unassigned": "कोई शिक्षक नियुक्त नहीं",
  "cover.reason.away": "शिक्षक अनुपस्थित",
  "severity.error": "सुधार आवश्यक",
  "severity.warning": "देखें",
  "severity.info": "सूचना",
  "staff.status.active": "कार्यरत",
  "staff.status.inactive": "निष्क्रिय",
  "staff.status.resigned": "त्यागपत्र",
  "staff.status.retired": "सेवानिवृत्त",
  "staff.status.terminated": "सेवामुक्त",
  "certificate.kind.transfer": "स्थानांतरण प्रमाणपत्र",
  "certificate.kind.bonafide": "बोनाफाइड प्रमाणपत्र",
  "certificate.kind.character": "चरित्र प्रमाणपत्र",
  "certificate.kind.study": "अध्ययन प्रमाणपत्र",
  "certificate.kind.conduct": "आचरण प्रमाणपत्र",
  "certificate.kind.custom": "अन्य",
  "homework.due.today": "आज देय",
  "homework.due.tomorrow": "कल देय",
  "homework.due.yesterday": "कल देय था",
  "homework.due.inDays.one": "{count} दिन में देय",
  "homework.due.inDays.other": "{count} दिन में देय",
  "homework.due.overdue.one": "{count} दिन विलंबित",
  "homework.due.overdue.other": "{count} दिन विलंबित",
  "hr.method.bank_transfer": "बैंक हस्तांतरण",
  "hr.method.cash": "नकद",
  "hr.method.cheque": "चेक",
  "hr.method.other": "अन्य",
  "hr.attendance.present": "उपस्थित",
  "hr.attendance.absent": "अनुपस्थित",
  "hr.attendance.half_day": "आधा दिन",
  "hr.attendance.on_leave": "अवकाश पर",
  "hr.attendance.on_duty": "ड्यूटी पर",
  "hr.attendance.unmarked": "अंकित नहीं",
  "hr.leaveStatus.pending": "निर्णय प्रतीक्षित",
  "hr.leaveStatus.approved": "स्वीकृत",
  "hr.leaveStatus.rejected": "अस्वीकृत",
  "hr.leaveStatus.cancelled": "वापस लिया",
  "hr.runStatus.draft": "प्रारूप",
  "hr.runStatus.finalised": "अंतिम",
  "hr.runStatus.discarded": "रद्द",
  "accounts.type.asset": "परिसंपत्ति",
  "accounts.type.liability": "देयता",
  "accounts.type.equity": "पूंजी",
  "accounts.type.income": "आय",
  "accounts.type.expense": "व्यय",
  "accounts.voucherStatus.draft": "प्रारूप",
  "accounts.voucherStatus.posted": "प्रविष्ट",
  "accounts.voucherStatus.void": "निरस्त",
  "accounts.source.manual": "जर्नल",
  "accounts.source.fee_ledger": "शुल्क रसीद",
  "accounts.source.payroll_payment": "वेतन भुगतान",
  "accounts.source.reversal": "प्रतिवर्तन",
  "fees.frequency.one_time": "एक बार",
  "fees.frequency.monthly": "मासिक",
  "fees.frequency.quarterly": "त्रैमासिक",
  "fees.frequency.annual": "वार्षिक",
  "concession.kind.percentage": "प्रतिशत",
  "concession.kind.amount": "निश्चित राशि",
  "concession.status.active": "प्रभावी",
  "concession.status.revoked": "वापस लिया",
  "stock.movement.receipt": "प्राप्त",
  "stock.movement.issue": "जारी",
  "stock.movement.return": "वापस",
  "stock.movement.adjustment": "समायोजन",
  "stock.movement.write_off": "बट्टे खाते",
  "nav.team": "लोग और आमंत्रण",
  "nav.plan": "योजना",
  "jobs.status.queued": "प्रतीक्षारत",
  "jobs.status.processing": "चल रहा है",
  "jobs.status.completed": "पूरा",
  "jobs.status.failed": "रुक गया",
  "jobs.status.refused": "रुक गया",
  "jobs.sentence.queued": "शुरू होने की प्रतीक्षा में। एक मिनट के भीतर शुरू होगा।",
  "jobs.sentence.carryingOn": "जारी है।",
  "jobs.sentence.running": "अभी चल रहा है।",
  "jobs.sentence.done": "पूरा हुआ।",
  "jobs.sentence.stopped": "यह रुक गया, और कोई कारण दर्ज नहीं हुआ।",
  "nav.jobs": "पृष्ठभूमि कार्य",
  "nav.permissions": "हर भूमिका क्या कर सकती है",
  "nav.arrangements": "बस और छात्रावास",

  // --- the office batch ----------------------------------------------------
  // --- the boundaries every route now has --------------------------------
  // --- identity cards ------------------------------------------------------
  "idCard.downloadPdf": "PDF डाउनलोड करें",
  "idCard.title": "पहचान पत्र",
  "idCard.subtitle": "एक कक्षा के लिए छापने योग्य कार्ड, एक शीट पर आठ।",
  "idCard.pickClass": "कक्षा",
  "idCard.print": "ये कार्ड छापें",
  "idCard.printOne": "यह कार्ड छापें",
  "idCard.validFor": "{session} के लिए मान्य",
  "idCard.admissionNumber": "प्रवेश क्रमांक",
  "idCard.roll": "अनुक्रमांक",
  "idCard.dateOfBirth": "जन्म तिथि",
  "idCard.bloodGroup": "रक्त समूह",
  "idCard.guardian": "अभिभावक",
  "idCard.emptyClass": "इस वर्ष उस कक्षा में कोई नामांकित नहीं है।",
  "idCard.tooMany":
    "उस कक्षा में {count} बच्चे हैं — यह पृष्ठ एक बार में {max} तक छापता है। दो भागों में छापें, या कक्षा छोटी करें।",
  "idCard.gap.photo": "कोई फ़ोटो नहीं",
  "idCard.gap.className": "इस वर्ष किसी कक्षा में नामांकित नहीं",
  "idCard.gap.guardianPhone": "अभिभावक का फ़ोन नंबर नहीं",
  "idCard.gap.guardianName": "अभिभावक दर्ज नहीं",
  "idCard.gap.bloodGroup": "रक्त समूह दर्ज नहीं",
  "idCard.gap.dateOfBirth": "जन्म तिथि दर्ज नहीं",
  "idCard.missingPhotos.one": "{count} कार्ड पर फ़ोटो नहीं है।",
  "idCard.missingPhotos.other": "{count} कार्डों पर फ़ोटो नहीं है।",
  "idCard.staffTitle": "कर्मचारी पहचान पत्र",
  "idCard.staffSubtitle": "यहाँ कार्यरत सभी के लिए छापने योग्य कार्ड, एक शीट पर आठ।",
  "idCard.employeeCode": "कर्मचारी क्रमांक",
  "idCard.phone": "फ़ोन",
  "idCard.department": "विभाग",
  "idCard.allDepartments": "सभी",
  "idCard.emptyStaff": "सूची में उससे कोई मेल नहीं खाता।",
  "idCard.gap.phone": "फ़ोन नंबर नहीं",
  "idCard.gap.department": "विभाग दर्ज नहीं",
  "idCard.staffWithheld": "आपकी भूमिका कर्मचारी सूची नहीं खोल सकती।",
  "idCard.photo.heading": "फ़ोटो",
  "idCard.photo.choose": "फ़ोटो अपलोड करें",
  "idCard.photo.replace": "बदलें",
  "idCard.photo.remove": "हटाएँ",
  "idCard.photo.none": "अभी कोई फ़ोटो नहीं",
  "idCard.photo.limit": "JPEG, PNG या WebP, {size} तक।",
  "idCard.photo.uploaded": "फ़ोटो सहेज ली गई।",
  "idCard.photo.removed": "फ़ोटो हटा दी गई।",
  "boundary.notFound.title": "हम उसे नहीं खोल सके",
  "boundary.notFound.body":
    "हो सकता है उसे हटा दिया गया हो, या वह आपके देखने के लिए न हो। यदि आप किसी लिंक से यहाँ आए हैं, तो जिस अभिलेख की ओर वह इशारा करता था वह अब मौजूद नहीं है।",
  "boundary.notFound.back": "डैशबोर्ड पर लौटें",
  "boundary.error.title": "यह पृष्ठ लोड नहीं हो सका",
  "boundary.error.body":
    "कुछ भी खोया नहीं है। यह पृष्ठ पढ़ने में समस्या है, आपके विद्यालय के अभिलेखों में नहीं।",
  "boundary.error.reference": "संदर्भ {digest}",
  "boundary.error.retry": "फिर से कोशिश करें",
  "boundary.loading": "लोड हो रहा है",
  "academics.subjectKind.theory": "सैद्धांतिक",
  "academics.subjectKind.practical": "प्रायोगिक",

  "audit.action.insert": "बनाया गया",
  "audit.action.update": "बदला गया",
  "audit.action.delete": "हटाया गया",

  "checks.status.error": "चल नहीं सका",
  "checks.status.attention": "ध्यान चाहिए",
  "checks.status.ok": "कुछ बताने को नहीं",
  "checks.status.skipped": "आपकी भूमिका के लिए नहीं",

  "frontOffice.source.walk_in": "स्वयं आए",
  "frontOffice.source.phone": "फ़ोन किया",
  "frontOffice.source.website": "वेबसाइट",
  "frontOffice.source.referral": "किसी के कहने पर",
  "frontOffice.source.advertisement": "विज्ञापन",
  "frontOffice.source.other": "अन्य",
  "frontOffice.stage.new": "नया",
  "frontOffice.stage.contacted": "संपर्क हुआ",
  "frontOffice.stage.visited": "स्कूल देखा",
  "frontOffice.stage.applied": "आवेदन किया",
  "frontOffice.stage.admitted": "प्रवेश हुआ",
  "frontOffice.stage.lost": "कहीं और गए",

  "hostel.kind.boys": "बालक",
  "hostel.kind.girls": "बालिका",
  "hostel.kind.mixed": "मिश्रित",

  "promotion.decision.promote": "अगली कक्षा",
  "promotion.decision.repeat": "वही कक्षा",
  "promotion.decision.graduate": "विद्यालय पूर्ण",
  "promotion.decision.hold": "रोकें",
  "promotion.decisionHint.promote": "आने वाले सत्र में एक कक्षा ऊपर जाते हैं।",
  "promotion.decisionHint.repeat": "आने वाले सत्र में उसी कक्षा में रहते हैं।",
  "promotion.decisionHint.graduate": "पूर्व छात्र के रूप में विद्यालय छोड़ते हैं। कोई नया नामांकन नहीं।",
  "promotion.decisionHint.hold": "कुछ नहीं होता। पिछला नामांकन खुला रहता है।",
  "promotion.onMissingResult.hold": "रोक रखें",
  "promotion.onMissingResult.promote": "फिर भी अगली कक्षा",
  "promotion.onMissingResult.repeat": "वही कक्षा दोहराएँ",
  "promotion.onMissingResultHint.hold": "सुरक्षित उत्तर: किसी को देखना ही होगा।",
  "promotion.onMissingResultHint.promote": "परिणाम न होने को उत्तीर्ण मानें।",
  "promotion.onMissingResultHint.repeat": "परिणाम न होने को अनुत्तीर्ण मानें।",
  "promotion.leftBehind.library": "पुस्तकालय",
  "promotion.leftBehind.balance": "राशि",
  "promotion.leftBehind.other": "टिप्पणी",
  "promotion.currently": "अभी: {decision}",
  "promotion.tally": "{count} {decision}",

  "renewals.kind.transport": "बस सीटें",
  "renewals.kind.hostel": "छात्रावास बिस्तर",
  "renewals.blurb.transport":
    "इस वर्ष सीट वाले हर बच्चे को, अगले वर्ष के मार्ग पर उसी ठहराव से जोड़ा गया।",
  "renewals.blurb.hostel": "इस वर्ष बिस्तर वाले हर बच्चे के लिए, उसी कमरे का प्रस्ताव।",

  "schedules.kind.attendance.absentees": "अनुपस्थिति सूचना",
  "schedules.kind.fees.due_reminder": "शुल्क स्मरण",
  "schedules.kind.library.overdue": "पुस्तक वापसी स्मरण",
  "schedules.kind.report.digest": "निर्धारित रिपोर्ट",
  "schedules.kindDescription.attendance.absentees":
    "जिस दिन बच्चा अनुपस्थित हो, उसी दिन परिवार को बताता है। जिस दिन उपस्थिति दर्ज ही नहीं हुई, उस दिन कुछ नहीं भेजा जाता।",
  "schedules.kindDescription.fees.due_reminder":
    "जिन विद्यार्थियों पर राशि बकाया है, उनके परिवार को बताता है। न्यूनतम राशि तय करें ताकि दो रुपये के अंतर पर संदेश न जाए।",
  "schedules.kindDescription.library.overdue":
    "जिन विद्यार्थियों के पास नियत तिथि के बाद भी पुस्तक है, उनके परिवार को बताता है। कर्मचारियों का निपटारा वेतन से होता है, वे इसमें नहीं हैं।",
  "schedules.kindDescription.report.digest":
    "हर सुबह एक रिपोर्ट चलाता है और बताता है कि उसमें कितनी पंक्तियाँ मिलीं। यह आपके ही अधिकारों के साथ चलती है और उत्तर केवल आपको मिलता है — इसीलिए इसमें गिनती होती है, किसी का विवरण नहीं।",
  "schedules.cadence.daily": "रोज़, {time} बजे",
  "schedules.cadence.weekdays": "हर कार्यदिवस, {time} बजे",
  "schedules.cadence.mondayToSaturday": "सोमवार से शनिवार, {time} बजे",
  "schedules.cadence.weekends": "सप्ताहांत पर, {time} बजे",
  "schedules.cadence.days": "{days}, {time} बजे",
  "schedules.cadence.dayOfMonth": "हर माह की {day} तारीख़ को, {time} बजे",
  "schedules.grace.minutes": "{minutes} मिनट से अधिक देर होने पर छोड़ दिया जाएगा",
  "schedules.grace.hours": "{hours} घंटे से अधिक देर होने पर छोड़ दिया जाएगा",
  "schedules.grace.oneHour": "एक घंटे से अधिक देर होने पर छोड़ दिया जाएगा",
  "schedules.grace.day": "एक दिन से अधिक देर होने पर छोड़ दिया जाएगा",

  // Five ordinal categories, not one. `Intl.PluralRules("hi-IN", { type:
  // "ordinal" })` carves 1 / 2,3 / 4 / 6 / the rest into one, two, few, many,
  // other -- because Hindi ordinals are distinct *words* (पहला, दूसरा, तीसरा,
  // चौथा, छठा), not suffixes. Every one of them needs a key here: a category
  // with no entry falls back to the English catalogue, and the English entry is
  // "{n}st" -- so a missing key does not print English, it prints an English
  // *rule* applied to a Hindi sentence. That is what shipped for one test run:
  // "हर माह की 1st तारीख़ को".
  //
  // The date is written as a numeral in this frame, so all five are the bare
  // number and the sentence around it carries the meaning.
  "schedules.ordinal.one": "{n}",
  "schedules.ordinal.two": "{n}",
  "schedules.ordinal.few": "{n}",
  "schedules.ordinal.many": "{n}",
  "schedules.ordinal.other": "{n}",
  "transport.direction.both": "दोनों ओर",
  "transport.direction.pickup": "केवल लाना",
  "transport.direction.drop": "केवल छोड़ना",
  // --- the public application form (0268) -------------------------------
  "apply.title": "{college} में आवेदन करें",
  "apply.subtitle": "{session} शैक्षणिक वर्ष के लिए। खाते की आवश्यकता नहीं है — प्रवेश कार्यालय अगले चरणों के बारे में आपसे संपर्क करेगा।",
  "apply.closed.title": "आवेदन अभी खुले नहीं हैं",
  "apply.closed.body": "यह महाविद्यालय अभी ऑनलाइन आवेदन नहीं ले रहा है। कृपया सीधे महाविद्यालय से संपर्क करें।",
  "apply.section.child": "बच्चे के बारे में",
  "apply.section.contact": "महाविद्यालय किससे संपर्क करे?",
  "apply.firstName": "पहला नाम",
  "apply.lastName": "उपनाम",
  "apply.dateOfBirth": "जन्म तिथि",
  "apply.gender": "लिंग",
  "apply.gender.choose": "चुनें…",
  "apply.gender.male": "पुरुष",
  "apply.gender.female": "महिला",
  "apply.gender.other": "अन्य",
  "apply.gender.undisclosed": "बताना नहीं चाहते",
  "apply.classLevel": "किस कक्षा के लिए आवेदन",
  "apply.classLevel.none": "अभी तय नहीं",
  "apply.contactName": "आपका नाम",
  "apply.relationship": "बच्चे से संबंध",
  "apply.relationshipHint": "जैसे: माता, पिता, अभिभावक, या स्वयं।",
  "apply.phone": "फ़ोन नंबर",
  "apply.email": "ईमेल पता",
  "apply.contactHint": "कम से कम एक दें, ताकि महाविद्यालय उत्तर दे सके।",
  "apply.notes": "महाविद्यालय को और कुछ बताना हो",
  "apply.optional": "वैकल्पिक",
  "apply.submit": "आवेदन भेजें",
  "apply.submitting": "भेजा जा रहा है…",
  "apply.privacy": "आपकी जानकारी केवल {college} के प्रवेश कार्यालय को जाती है।",
  "apply.done.title": "आवेदन प्राप्त हुआ",
  "apply.done.body": "{college} को आपका आवेदन मिल गया है। यह संदर्भ संख्या संभालकर रखें — संपर्क करते समय कार्यालय इसे पूछेगा।",
  "apply.done.duplicate": "यह आवेदन आज पहले ही महाविद्यालय तक पहुँच चुका था, इसलिए दोबारा नहीं भेजा गया। संदर्भ संख्या वही है।",
  "apply.done.reference": "संदर्भ संख्या",
  "apply.done.another": "दूसरे बच्चे के लिए आवेदन करें",
  "apply.error.fields": "चिह्नित फ़ील्ड जाँचें।",
  "apply.error.busy": "पिछले एक घंटे में महाविद्यालय को बहुत से आवेदन मिले हैं। कृपया बाद में फिर प्रयास करें, या सीधे महाविद्यालय से संपर्क करें।",
  "apply.error.generic": "आपका आवेदन भेजा नहीं जा सका। कृपया फिर प्रयास करें।",
  "apply.error.firstName": "बच्चे का पहला नाम लिखें।",
  "apply.error.contactName": "अपना नाम लिखें।",
  "apply.error.contact": "फ़ोन नंबर या ईमेल पता दें।",
  "apply.error.phone": "केवल अंक, स्पेस और + का उपयोग करें।",
  "apply.error.email": "यह ईमेल पता सही नहीं लगता।",
  "apply.error.dateOfBirth": "सही जन्म तिथि लिखें।",
  "apply.error.tooLong": "यह बहुत लंबा है।",
  "apply.error.classLevel": "सूची में से कक्षा चुनें।",
  // --- live classes (0270) -------------------------------------------------
  "liveClasses.title": "लाइव कक्षाएँ",
  "liveClasses.subtitle": "ऑनलाइन पढ़ाए जाने वाले पाठ, और हर एक से जुड़ने का लिंक। लिंक पाठ शुरू होने से 15 मिनट पहले खुलता है।",
  "liveClasses.upcoming": "आज और आगे",
  "liveClasses.earlier": "पहले",
  "liveClasses.empty": "पिछले एक सप्ताह या अगले दो सप्ताह में कोई लाइव पाठ नहीं।",
  "liveClasses.join": "जुड़ें",
  "liveClasses.opensAt": "{time} पर खुलेगा",
  "liveClasses.ended": "समाप्त",
  "liveClasses.cancelledBecause": "रद्द: {reason}",
  "liveClasses.withTeacher": "{name} के साथ",
  "liveClasses.status.scheduled": "निर्धारित",
  "liveClasses.status.cancelled": "रद्द",
  "liveClasses.noAccess": "आपकी भूमिका लाइव कक्षाएँ नहीं देखती। ज़रूरत हो तो किसी व्यवस्थापक से पूछें।",
  "onlineTests.title": "ऑनलाइन परीक्षा",
  "onlineTests.subtitle": "आपकी कक्षाओं के लिए बहुविकल्पीय परीक्षाएँ, जो छात्र के जमा करते ही जाँच दी जाती हैं।",
  "onlineTests.familySubtitle": "आपकी कक्षा के लिए निर्धारित परीक्षाएँ। हर परीक्षा की घड़ी शुरू करते ही चलने लगती है।",
  "onlineTests.noAccess": "आपकी भूमिका में ऑनलाइन परीक्षाएँ शामिल नहीं हैं।",
  "onlineTests.empty": "अभी कोई परीक्षा नहीं। जिस कक्षा को आप पढ़ाते हैं, उसके लिए एक बनाइए।",
  "onlineTests.familyEmpty": "आपकी कक्षा के लिए कोई ऑनलाइन परीक्षा निर्धारित नहीं है।",
  "onlineTests.state.not_started": "शुरू नहीं किया",
  "onlineTests.state.in_progress": "जारी है",
  "onlineTests.state.submitted": "जमा किया",
  "onlineTests.state.lapsed": "समय समाप्त",
  "idCard.scanLabel": "इस कार्ड का स्कैन कोड",
};
