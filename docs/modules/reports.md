...
Sixteen reports ship today across ten modules, and the runner renders all of
them without knowing anything about any of them. Eight of the sixteen were added
after this page was written and none of them needed a line changed in it —
`hr.staff_attendance`, `hr.teacher_summary` and `exams.results` in migration
`0126`, `certificates.register` in `0135`, `schedules.runs` in `0143`,
`notices.reach` in `0146`, `attendance.student_leave` in `0150` and
`attendance.gaps` in `0152`. That is the property the catalog exists for. See
`docs/modules/dashboard.md`, `docs/modules/certificates.md`,
`docs/modules/schedules.md`, `docs/modules/notices.md`,
`docs/modules/student-leave.md` and `docs/modules/attendance.md`.
...
isolation depended on each of sixteen functions remembering, the seventeenth
